...
visitors %>%
  group_by(region) %>%
  summarise(attraction_count = n()) %>%
  filter(attraction_count > 50) %>%
  count()
```
...
visitors %>%
  summarise(
    mean_2021 = mean(as.numeric(n_2021), na.rm = TRUE),
    med_2021 = median(as.numeric(n_2021), na.rm = TRUE)
  )
```
...
**NA means not available so the data is missing**
